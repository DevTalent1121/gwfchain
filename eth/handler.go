...
import (
	...
	"github.com/NlaakStudiosLLC/gwfchain/v3/eth/fetcher"
...
// NewProtocolManager returns a new ethereum sub protocol manager. The GWFChain sub protocol manages peers capable
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing GWFChain peer", "peer", id)

	// Unregister the peer from the downloader and GWFChain peer set
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping GWFChain protocol")
	...
	log.Info("GWFChain protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("GWFChain peer connected", "name", p.Name())

	// Execute the GWFChain handshake
	var (
	...
	if err := p.Handshake(pm.networkId, td, hash, genesis.Hash()); err != nil {
		p.Log().Debug("GWFChain handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("GWFChain peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Error("GWFChain message handling failed", "err", err)
			return err
		}
	}
}
...
// NodeInfo represents a short summary of the GWFChain sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // GWFChain network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
