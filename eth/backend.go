...
// Package eth implements the GWFChain protocol.
package eth
...
import (
	...
	"github.com/NlaakStudiosLLC/gwfchain/v3/core/bloombits"
	...
	"github.com/NlaakStudiosLLC/gwfchain/v3/eth/filters"
	"github.com/NlaakStudiosLLC/gwfchain/v3/eth/gasprice"
	"github.com/NlaakStudiosLLC/gwfchain/v3/internal/ethapi"
	"github.com/NlaakStudiosLLC/gwfchain/v3/log"
	"github.com/NlaakStudiosLLC/gwfchain/v3/miner"
...
// GWFChain implements the GWFChain full node service.
type GoChain struct {
	config      *Config
...
// New creates a new GWFChain object (including the
// initialisation of the common GWFChain object)
func New(sctx *node.ServiceContext, config *Config) (*GoChain, error) {
	if config.SyncMode == downloader.LightSync {
		return nil, errors.New("can't run eth.GWFChain in light sync mode, use les.LightGWFChain")
	}
	...
	log.Info("Initialising GWFChain protocol", "versions", ProtocolVersions, "network", config.NetworkId, "dbversion", dbVer)

	if !config.SkipBcVersionCheck {
		if bcVersion != nil && *bcVersion > core.BlockChainVersion {
			return nil, fmt.Errorf("database version is v%d, GWFChain %s only supports v%d", *bcVersion, params.Version, core.BlockChainVersion)
...
// GWFChain protocol implementation.
