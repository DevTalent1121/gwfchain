...
type wallet struct {
	...
	deriveChain     gwfchain.ChainStateReader // Blockchain state reader to discover used account with
...
// chain state reader.
func (w *wallet) SelfDerive(bases []accounts.DerivationPath, chain gwfchain.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
