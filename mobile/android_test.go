...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", GoChain.LightScryptN, GoChain.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.gwfchain", new NodeConfig());
			node.start();
...
func TestAndroid(t *testing.T) {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "io.gwfchain", "github.com/NlaakStudiosLLC/gwfchain/v3/mobile")
	...
	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "io", "gwfchain", "gethtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
